...
def button_ask3():
    ...
    Label(c, text="Выберите тип модуляции").grid(row=0, column=3)
    var = IntVar()
    var.set(0)
...
def button_ask9():
    ...
    if modulation_type == 'Амплитудная манипуляция':
        ...
        modulated_signal = ASK(10, bits_encode)
        ...
        demodulated_signal = ASDK(10, modulated_signal)
...
button9 = Button(window, text='Запустить моделирование', command=button_ask9).grid(row=4, column=3)
